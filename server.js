// ============================================================
//  PUZZLE #71 SCANNER
//  BTC ONLY | 50 KEYS/BATCH | 100 ADDRESSES/BATCH | NO REPEAT
...
//  PUZZLE #71 RANGE
// ============================================================
const RANGE_MIN = 1n << 66n;           // 400000000000000000
const RANGE_MAX = (1n << 67n) - 1n;    // 7fffffffffffffffff
const BATCH_SIZE = 50;
const generatedKeys = new Set();       // no repeat tracking
...
function genUniquePrivKey() {
    ...
    do {
        ...
        attempts++;
        if (attempts > 10000) { generatedKeys.clear(); console.log('[WARN] Key Set cleared (memory)'); attempts = 0; }
...
const BTC_RPC = [];
while (BTC_RPC.length < 150) BTC_RPC.push(...BASE_BTC);
const RPC = BTC_RPC.slice(0, 150);
...
async function checkBalance(addr) {
    ...
    while (tried.size < total) {
        ...
        const results = await Promise.allSettled(batch.map(i => callBTC(RPC[i], addr).catch(() => ({...ERR}))));
...
// ============================================================
//  CONCURRENCY LIMITER (max 20 parallel API calls)
// ============================================================
...
// ============================================================
//  PROCESS BATCH: 50 keys → 100 addresses → parallel check
// ============================================================
async function processBatch() {
    ...
    // Build 100 address check tasks
    const tasks = [];
    for (const w of wallets) {
        tasks.push(() => checkBalance(w.comp_addr));
        tasks.push(() => checkBalance(w.uncomp_addr));
    }

    // Run 100 checks with max 20 parallel
    const results = await parallelLimit(tasks, 20);
    state.addrChecked += 100;

    // Check for found balances
    let batchFound = [];
    for (let i = 0; i < wallets.length; i++) {
        const w = wallets[i];
        const compR = results[i * 2];
        const uncompR = results[i * 2 + 1];

        // Compressed
        if (compR && !compR.error && ((compR.received||0) > 0 || (compR.sent||0) > 0 || (compR.balance||0) > 0)) {
            state.foundCount++;
            ...
            batchFound.push(entry);
            console.log('[FOUND] #' + entry.idx + ' BTC COMP B:' + entry.balance.toFixed(8));
        }

        // Uncompressed
        if (uncompR && !uncompR.error && ((uncompR.received||0) > 0 || (uncompR.sent||0) > 0 || (uncompR.balance||0) > 0)) {
            state.foundCount++;
    ...
    // Emit batch to viewers
    const batchData = wallets.map((w, i) => ({
        privkey_hex: w.privkey_hex,
        comp_addr: w.comp_addr,
        uncomp_addr: w.uncomp_addr,
        comp: results[i * 2] || {...ERR},
        uncomp: results[i * 2 + 1] || {...ERR}
    }));
...
io.on('connection', function(socket) {
    ...
    socket.emit('init', { checkCount:state.checkCount, foundCount:state.foundCount, apiCallCount, speed:state.speedValue, addrChecked:state.addrChecked, foundData:state.foundData.slice(-50) });
...
server.listen(PORT, function() {
    console.log('============================================');
    console.log('  PUZZLE #71 SCANNER');
    console.log('  BTC ONLY | 50 KEYS/BATCH | 100 ADDR/BATCH');
    console.log('  Range: 400000000000000000 - 7fffffffffffffffff');
    console.log('  Total combinations: 2^66 (~7.3e19)');
    console.log('  Generated keys tracked (no repeat)');
    console.log('  BTC APIs: ' + RPC.length + ' endpoints');
    console.log('============================================');
    io.emit('log', {msg:'<span style="color:#f97316;font-weight:900;font-size:14px">PUZZLE #71 SCANNER - BTC ONLY</span>'});
    io.emit('log', {msg:'<span style="color:#22c55e">50 Keys/Batch | 100 Addresses/Batch | No Repeat</span>'});
    io.emit('log', {msg:'<span style="color:#60a5fa">Range: 400000000000000000 - 7fffffffffffffffff (2^66)</span>'});
    io.emit('log', {msg:''});
