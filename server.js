// ============================================================
//  PUZZLE #71 SCANNER v14 — ZERO DELAY ENGINE
...
// Load found data from JSONL file on startup
function loadFoundData() {
    try {
        if (fs.existsSync('found_data.json')) {
            // JSONL format: one JSON object per line — crash-safe, no bracket issues
            ...
            if (parsed.length) {
                ...
                console.log('[LOAD] Found data loaded: ' + state.foundCount + ' entries from found_data.json');
            }
        }
    } catch(e) {
...
function saveFound(entry) {
    // JSONL format: one JSON per line — always valid, crash-safe
    try {
...
async function processBatch() {
    ...
    if (addrResults) {
        ...
        state.batchMiss++;
        // API FAILED — do NOT display anything, silently skip
        state.addrChecked += allAddrs.length;
        return;
    }

    // API SUCCESS — now display results
    state.addrChecked += allAddrs.length;
    ...
    for (var m = 0; m < wallets.length; m++) {
        ...
        // ONLY emit wallet when API check is successful
        io.emit('wallet', {
...
io.on('connection', function(socket) {
    ...
    socket.emit('init', {
        ...
        foundData: state.foundData, totalApis: BATCH_APIS.length,
...
async function boot() {
    // Load previous found data from file (JSONL — always valid)
    loadFoundData();

    console.log('============================================');
    console.log('  PUZZLE #71 SCANNER v14 — ZERO DELAY');
    ...
    io.emit('log', { msg: '<span style="color:#f97316;font-weight:900;font-size:14px">PUZZLE #71 SCANNER v14 — ZERO DELAY ENGINE</span>' });
    ...
    io.emit('log', { msg: '<span style="color:#facc15">FOUND = Any R>0 OR S>0 OR B>0 → Save forever</span>' });
    io.emit('log', { msg: '<span style="color:#64748b">D = Real live viewer count</span>' });
    io.emit('log', { msg: '<span style="color:#f472b6">ERR hidden — only successful API checks shown</span>' });
    io.emit('log', { msg: '<span style="color:#a78bfa">FOUND data persists in found_data.json (JSONL)</span>' });
    if (state.foundCount > 0) {
        io.emit('log', { msg: '<span style="color:#22c55e;font-weight:700">Loaded ' + state.foundCount + ' previously found entries</span>' });
    }
    io.emit('log', { msg: '' });
