// ============================================================
//  PUZZLE #71 SCANNER v2
//  BTC ONLY | 50 KEYS/BATCH | REAL-TIME DISPLAY
//  150+ APIs | PROXY ROTATION | NO REPEAT
// ============================================================
...
// ============================================================
//  PROXY ROTATION - Add your proxies here (different IP per call)
//  Format: 'http://user:pass@host:port' or 'http://host:port'
//  Free proxy lists: https://free-proxy-list.net/
//  Paid rotating: smartproxy, brightdata, oxylabs
// ============================================================
const PROXIES = [
...
// ============================================================
//  FETCH WITH PROXY + TIMEOUT (800ms fast fail)
// ============================================================
let apiCallCount = 0;
const ZERO = {received:0,sent:0,balance:0,error:false};
const ERR  = {received:0,sent:0,balance:0,error:true};

async function fetchT(url, opts = {}, ms = 800) {
    const proxy = getNextProxy();
    const fetchOpts = { ...opts, timeout: ms, ...(proxy ? { agent: proxy } : {}) };
    try {
        const r = await nodeFetch(url, fetchOpts);
        return r;
    } catch(e) { throw e; }
}
...
// ============================================================
//  FAST BALANCE CHECK (2 random APIs, 800ms timeout each)
// ============================================================
...
async function checkBalance(addr) {
    ...
    // Try 2 APIs simultaneously
    const batch = shuffled.slice(0, 2);
    const results = await Promise.allSettled(batch.map(ep => callBTC(ep, addr).catch(() => ({...ERR}))));
    for (const r of results) if (r.status === 'fulfilled' && r.value && !r.value.error) return r.value;
    // If both fail, try 2 more
    const batch2 = shuffled.slice(2, 4);
    const results2 = await Promise.allSettled(batch2.map(ep => callBTC(ep, addr).catch(() => ({...ERR}))));
    for (const r of results2) if (r.status === 'fulfilled' && r.value && !r.value.error) return r.value;
    return {...ERR};
}
...
// ============================================================
//  CONCURRENCY LIMITER (max 40 parallel API calls)
// ============================================================
...
async function processBatch() {
    ...
    // Run all 100 checks with max 40 parallel
    const [compResults, uncompResults] = await Promise.all([
        parallelLimit(compTasks, 40),
        parallelLimit(uncompTasks, 40)
    ]);
...
io.on('connection', function(socket) {
    ...
    socket.emit('init', { checkCount:state.checkCount, foundCount:state.foundCount, apiCallCount, speed:state.speedValue, addrChecked:state.addrChecked, foundData:state.foundData.slice(-50), totalApis:TOTAL_APIS, proxyCount:PROXIES.length });
...
server.listen(PORT, function() {
    console.log('============================================');
    console.log('  PUZZLE #71 SCANNER v2');
    console.log('  BTC ONLY | 50 Keys/Batch | 100 Addr/Batch');
    console.log('  APIs: ' + TOTAL_APIS + ' | Timeout: 800ms');
    console.log('  Proxies: ' + (PROXIES.length ? PROXIES.length + ' (rotating)' : 'NONE (add to PROXIES[])'));
    console.log('  Range: 400000000000000000 - 7fffffffffffffffff');
    console.log('============================================');
    io.emit('log', {msg:'<span style="color:#f97316;font-weight:900;font-size:14px">PUZZLE #71 SCANNER v2 - BTC ONLY</span>'});
    io.emit('log', {msg:'<span style="color:#22c55e">50 Keys/Batch | 100 Addr/Batch | Real-Time Display</span>'});
    io.emit('log', {msg:'<span style="color:#60a5fa">APIs: '+TOTAL_APIS+' | Timeout: 800ms | Parallel: 40</span>'});
    io.emit('log', {msg:'<span style="color:#22d3ee">Proxies: '+(PROXIES.length ? PROXIES.length+' (rotating per call)' : 'Add to PROXIES[] for IP rotation')+'</span>'});
    io.emit('log', {msg:''});
